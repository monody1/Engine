...
int engine::VulkanApplication::Initialize(){
    int res = BaseApplication::Initialize();
    glfwInit();
    glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
    glfwWindowHint(GLFW_RESIZABLE, GL_FALSE);
    m_config.window_title = "Hello Vulkan!";
...
void engine::VulkanApplication::Finalize() {
    CleanUpVulkan();
...
void engine::VulkanApplication::CloseCallback(GLFWwindow *window) {
    BaseApplication::m_quit = true;
}

void engine::VulkanApplication::Tick() {
    glfwPollEvents();
    DrawFrame();
    vkDeviceWaitIdle(m_device);
}

bool engine::VulkanApplication::IsQuit() {
    return m_quit;
}

void engine::VulkanApplication::InitVulkan() {
    CreateInstance();
    SetupDebugCallback();
    CreateSurface();
    PickPhysicalDevice();
...
void engine::VulkanApplication::CreateSemaphores() 
{
...
void engine::VulkanApplication::DrawFrame() 
{
    uint32_t imageIndex;
...
engine::QueueFamilyIndices engine::VulkanApplication::FindQueueFamilies(VkPhysicalDevice device){
...
bool engine::VulkanApplication::CheckDeviceExtensionSupport(VkPhysicalDevice device){
...
bool engine::VulkanApplication::IsDeviceSuitable(VkPhysicalDevice device){
...
void engine::VulkanApplication::PickPhysicalDevice(){
...
void engine::VulkanApplication::CreateSurface(){
    if(glfwCreateWindowSurface(m_instance, m_pWindow, nullptr, &m_surface) != VK_SUCCESS){
        throw std::runtime_error("failed to create surface!");
    }
}

void engine::VulkanApplication::CreateLogicalDevice(){
...
void engine::VulkanApplication::CreateFrameBuffers() 
{   
...
void engine::VulkanApplication::CreateCommandPool() 
{
    QueueFamilyIndices indices = FindQueueFamilies(m_physicalDevice);  
...
void engine::VulkanApplication::CreateCommandBuffers() 
{
...
void engine::VulkanApplication::CreateSwapChain() 
{
...
void engine::VulkanApplication::CreateImageViews() 
{
...
VkShaderModule engine::VulkanApplication::CreateShaderModule(char* buffer, size_t bufferSize) 
{
...
void engine::VulkanApplication::CreateRenderPass() 
{
...
void engine::VulkanApplication::CreateGraphicsPipeline() 
{
    ...
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    viewport.width = m_config.screen_width;
    viewport.height = m_config.screen_height;
...
std::vector<const char*> engine::VulkanApplication::GetRequiredExtensions(){
...
VkSurfaceFormatKHR engine::VulkanApplication::ChooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR> &availableFormats) 
{
...
VkPresentModeKHR engine::VulkanApplication::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR> &availablePresentModes) 
{
...
VkExtent2D engine::VulkanApplication::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) 
{
...
void engine::VulkanApplication::CreateInstance(){
...
VkResult engine::VulkanApplication::CreateDebugUtilsMessengerEXT(
    VkInstance instance, 
...
void engine::VulkanApplication::DestroyDebugUtilsMessengerEXT(
    VkInstance instance, 
...
void engine::VulkanApplication::PopulateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& debugCreateInfo){
    debugCreateInfo = {};
...
void engine::VulkanApplication::SetupDebugCallback(){
...
void engine::VulkanApplication::CleanUpVulkan(){
...
bool engine::VulkanApplication::CheckValidationLayerSupport(){
...
VKAPI_ATTR VkBool32 VKAPI_CALL engine::VulkanApplication::DebugCallback(
...
engine::SwapChainSupportDetails engine::VulkanApplication::QuerySwapChainSupport(VkPhysicalDevice device){
